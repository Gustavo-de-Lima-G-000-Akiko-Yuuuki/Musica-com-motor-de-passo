...
// Esta classe mantém o estado do cabeçote de reprodução de uma única parte da música. O método `tick()` pode ser chamado
// tantas vezes por segundo quanto possível, dado o tempo atual em microssegundos. É responsável por mover o
// motor de passo atribuído a ele no momento preciso para fazer ressoar as notas.
class PlayThread {
public:
  PlayThread(int pino, int passosTotais, int tempo[], int frequencia[]);
  void fatiaDeTempo(unsigned long agora);
...
private:
  ...
  int passosTotais;
  int passoAtual;
  unsigned long proximoTempoPasso;
  unsigned long proximoTempoTique;
};

PlayThread::PlayThread(int _pino, int _passosTotais, int _tempo[], int _frequencia[]) {
  pino = _pino;
  tempo = _tempo;
  frequencia = _frequencia;
  passosTotais = _passosTotais;
  passoAtual = -1;
  proximoTempoPasso = 0;
  proximoTempoTique = 0;
}

void PlayThread::fatiaDeTempo(unsigned long agora) {
  if (agora > proximoTempoPasso) {
    // Hora da próxima ação
    passoAtual += 1;
    if (passoAtual > passosTotais) {
      passoAtual = 0;
    }

    unsigned long tempoPasso = ((unsigned long)pgm_read_word_near(tempo + passoAtual)) * 1000L;
    int frequenciaPasso = pgm_read_word_near(frequencia + passoAtual);

    proximoTempoPasso = agora + tempoPasso;
    if (frequenciaPasso == 0) {
      proximoTempoTique = proximoTempoPasso;
    } else {
      tique();
      proximoTempoTique = micros() + (unsigned long)frequenciaPasso;
    }
    // Serial.println("---");
    // Serial.println(passoAtual);
  } else if (agora > proximoTempoTique) {
    // Continuar ação existente, hora de mover o motor
    int frequenciaPasso = pgm_read_word_near(frequencia + passoAtual);
    tique();
    proximoTempoTique = micros() + (unsigned long)frequenciaPasso;
  }
...
void PlayThread::tique() {
  digitalWrite(pino, HIGH);
  delayMicroseconds(5);
  digitalWrite(pino, LOW);
}

// Três partes concorrentes
...
void loop() {
  // Quanto mais rápido fizermos isso, melhor soará as notas, a precisão é fundamental
  thread1.fatiaDeTempo(micros());
  thread2.fatiaDeTempo(micros());
  thread3.fatiaDeTempo(micros());
}
